...
class KipApi implements KipApiInterface
{
            ...
            if ($id = $this->_customer->getId() ?? ($this->_userContext->getUserId() ?? false)) {
                ...
                $this->jsonResponse([
                    ...
                    "data" => [
                        ...
                        "global" => [
                            "bins" => $binsBAC . ',' . $binsCUS . ',' . $binsPROME,
    ...
    private function getCurrentCoupon($quote, $bin) {
        ...
        if ($checkApplyBin) {
            if ($couponToApply == "" or $couponToApply == null or empty($couponToApply)) {
                $this->_logger->addInfo(print_r(' >1'.strval($couponToApply), true));
                if(!$BINCoupon = $this->getBINCoupon($quote, $bin, $BACBINcoupons, $BACBINS)) {
                    $this->_logger->addInfo(print_r('--1', true));
                    if(!$BINCoupon = $this->getBINCoupon($quote, $bin, $CUSBINcoupons, $CUSBINS)) {
                        $this->_logger->addInfo(print_r('--2', true));
                        $BINCoupon = $this->getBINCoupon($quote, $bin, $PROMEBINcoupons, $PROMEBINS);
                        $this->_logger->addInfo(print_r('--3'.strval($BINCoupon), true));
                    }
                }
            }
        }
        ...
        //-------------------------------------------------------------
        $this->_logger->addInfo(print_r('finally:'.$couponToApply, true));
    ...
    {
        $products = $this->_kip->getCheckoutImpulseProductsv2();
        $result = [];
        ...
        foreach ($products as $product) {
            ...
            foreach ($actualCartSkus as $productact) {
                if ($productact==$product->getSku()) {
