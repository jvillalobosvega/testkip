                ...
                if (is_array($kcis)) {
                    ...
                    foreach ($kcis as $id => &$kci) {
                        ...
                        if ($product->isSalable()) {
                            ...
                            $this->_logger->addInfo(print_r("UOA" . $update . '-' . $add, true));
                            if ($update || $add) {
                                if ($kci['q'] > $qtyLeft) {
                                    $kci['q'] = $qtyLeft;
                                    if ($kci['q'] > 0) {
                                ...
                                $this->_logger->addInfo(print_r("XXXDETECT::", true));
                                $this->_logger->addInfo(print_r($kci, true));
                                $this->_logger->addInfo(print_r($id, true));
                                $this->_logger->addInfo(print_r($add, true));
                                $this->_logger->addInfo(print_r("XXXDETECT---->", true));
                                if ($add) {
                                    ...
                                    try {
                                        ...
                                        $this->_logger->addInfo(print_r($id, true));
                                        //$kcis[$id]
                                        $this->_logger->addInfo(print_r($kcis[$id], true));
                                        $this->_logger->addInfo(print_r($configurable, true));
                                        if ($configurable) {
                                            $kci['c']['qty'] = $kci['q'];
                            ...
                            if ($update || $add) {
                                ...
                                if ($add) {
                                    ...
                                    try {
                                        ...
                                        $configurable = isset($kci['c']) ? ($kci['c'] ?? false) : false;
                                        if (str_contains($product->getSku(), '-')) {
                                            $configurable = true;
                                        }
                                        ...
                                        if ($configurable) {
                                            ...
                                            try {
                                                ...
                                                $vnms = explode('-', $vname);
                                                $variantNameSelected = $vnms[1];
                                                $attributeNameSelected = "";
                                                $attributeIndexSelected = 0;
                                                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                                                $productId = $objectManager->get('Magento\Catalog\Model\Product')->getIdBySku($code[0]);
                                                try {
                                                    ...
                                                    foreach ($data as $attr) {
                                                        foreach ($attr as $p) {
                                                            ...
                                                            if ($p['option_title'] == $variantNameSelected) {
                                                                $attributeNameSelected = $p['attribute_code'];
                                                                $attributeIndexSelected = $p['value_index'];
                                                            }
                                                        }
                                                    }
                                                    $json_string = json_encode($options, JSON_PRETTY_PRINT);
                                                    $this->_logger->addInfo(print_r($json_string, true));
                                                    $attributeId = $this->_eavAttribute->getIdByCode('catalog_product', $attributeNameSelected);
                                                    $px = $quote->addProduct($_product, new \Magento\Framework\DataObject([
                                                        'qty' => $kci['q'],
                                                        'super_attribute' => [$attributeId => $attributeIndexSelected]
                                                    ]));
                                                    unset($kcis[$id]);
                                                } catch (\Exception $e) {
                                                    $this->_logger->addInfo(print_r("ERROR909:--> " . $e, true));
                                                }
                                            }
                                        } else {
                                            $quote->addProduct($product, new \Magento\Framework\DataObject([
