 ...
 * @param pdp
 */
export const controlAtc = (atc, product, params = null, pdp = false) => {
    ...
    if (params) {
        ...
        renderCart(productId, 'add', false, JSON.parse(preCart));
    }
}
                ...
                if (!itemId) {
                    ...
                    switch (action) {
                        case 'add':
                            ...
                            // console.log("product:"+cart[itemId].n);
                            // console.log("product:"+cart[itemId].p);
                            // console.log("product:"+itemId);
                            //DATALAYER-CODITY ADDTOCART                            
                        ...
                        case 'delete':
                            ...
                            // console.log("product:"+cart[itemId].n);
                            // console.log("product:"+cart[itemId].p);
                            // console.log("product:"+cart[itemId].q);
                            //DATALAYER-CODITY UPDATECART
                            window.dataLayer.push({
 ...
 * @param lsoptions
 */
const syncCartBackend = (token, backToFront = false, lsoptions = null) => {
    let footerMiniCart = selectDoc('#minicart-custom-footer'),
        kipImpulse = selectDoc('#kip-impulse-wrapper');

    if (!window.kipatcAdding) {
        ...
        console.log('CODITY-FIX: '+lastExecution);
            ...
            if (data.status === 200) {
                ...
                if (!isNaN(expressLeftTotal)) {
                    ...
                    if (expressLeftTotal < 0) {
                        ...
                        expressCopy = `Express restantes: ${expressLeftTotal.toFixed(2)} ${expressLeftTotal < 0 ? '(no disponible)' : ''}`;
                ...
                if (!isNaN(flashLeftTotal)) {
                    ...
                    if (flashLeftTotal < 0) {
                        ...
                        flashCopy = `Flash restantes: ${flashLeftTotal} ${flashLeftTotal < 0 ? '(no disponible)' : ''}`;
