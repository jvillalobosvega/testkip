...
class Kipping implements KippingInterface
{
        ...
        if ($frontendUser || $adminRequest) {
            ...
            if ($order) {
                ...
                try {
                    ...
                    $crude_app = $manage["app"];
                    if ($crude_app != "null") {                                                
                        $platform = $crude_app." :mobile_phone:";
                    }
                } catch (\Exception $e) {
                ...
                //PLATFORM
                try {
                    $this->_logger->addInfo(print_r('PLATAFORMA', true));
                    if ($order->canComment()) {
                        if (isset($data['app'])) {
                            $history = $this->_orderHistoryFactory->create()
                                ->setEntityName(\Magento\Sales\Model\Order::ENTITY)
                                ->setComment(__('PLATAFORMA:' . $data['app']))
                                ->setIsCustomerNotified(false)
                                ->setStatus($order->getStatus())
                                ->setIsVisibleOnFront(false);
                            $order->addStatusHistory($history);
    ...
    public function syncJarvis(){
        $this->_logger->addInfo(print_r('Starting Jarvis Sync', true));
        $url = 'https://kip-logistic-api.azurewebsites.net/orders_sync';
        $curl = curl_init();        
    ...
    public function placeLsOrder($order, $isAdmin = false)
    {
        ...
        try {
            ...
            if ($this->lsr->isLSR($this->lsr->getCurrentStoreId())) {
                ...
                // $this->_logger->addInfo(print_r('-->DEBUG CONFIG ITEMS START',true));
                // $this->_logger->addInfo(print_r($order,true));
                // $this->_logger->addInfo(print_r('-->DEBUG CONFIG ITEMS =====',true));
                // $this->_logger->addInfo(print_r($oneListCalculation,true));
                // $this->_logger->addInfo(print_r('-->DEBUG CONFIG ITEMS END',true));
                        ...
                        try {
                            if ($response) {
                                ...
                                try{
                                    ...
                                    $this->_logger->addInfo(print_r('Jarvis API Not found: '.$e, true));
                                }
